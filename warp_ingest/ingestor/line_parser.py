...
stop_words.add("per")
# Generic legal-boilerplate recital openers. A line that begins with one of these
# is prose (a recital clause), never a heading. Content-free backstop kept tiny and
# universal (see prefer-structural-with-tunable-keyword-backstop rule).
_RECITAL_TRIGGERS = frozenset({"whereas", "now therefore"})
continuing_chars = "!\"&'+,./:;<=?@\\]^_`|}~"
list_chars = [
    "•",
    "➢",
...
class Word:
    ...
    def check_date(self):
        # Every date pattern below has a numeric field (%d / %y / %Y), so a token
        # with no digit can never match — gate on "has a digit" to skip the
        # expensive strptime storm on digit-free hyphenated words ("well-known",
        # "e-mail", "pre-existing").  Output-identical: the original loop would
        # ValueError on all patterns for such tokens, leaving is_date_entry False.
        self.is_date_entry = False
        if ("/" in self.text or "-" in self.text) and any(
            ch.isdigit() for ch in self.text
        ):
...
# ---------------------------------------------------------------------------
# Shared caches (perf).  Word and style-free Line construction are pure
# functions of the input string — deterministic, and nothing in the pipeline
# mutates the objects after construction (their JSON views are read-only too) —
# so identical tokens/texts can share one instance.  The engine re-parses the
# same block text in several phases (block typing, organize/indent, level
# assignment), so the hit rate is high.  Both caches are LRU-bounded so a
# long-lived service can't grow without bound, and ``WARP_LINE_CACHE=0``
# bypasses them entirely (each call then builds a fresh object, the historical
# behavior).
# ---------------------------------------------------------------------------
_LINE_CACHE_DISABLED = os.environ.get("WARP_LINE_CACHE", "").lower() in (
    "0",
    "false",
    "no",
)
...
def _cached_word(token):
    return Word(token)


def make_word(token):
    """A ``Word`` for *token*, shared via cache (see block comment above)."""
    if _LINE_CACHE_DISABLED:
        return Word(token)
    return _cached_word(token)
...
class Line:
    ...
    def parse_line(self):
        ...
        # Content-free prose signals. A genuine heading is Title Case / ALL CAPS and
        # starts with a capitalized word; body prose either continues syntactically
        # (ends with comma/conjunction), or *begins* with a lowercase word, or is a
        # recital clause opening with a generic legal-boilerplate trigger.
        first_alpha = ""
        for w in self.text.split():
            ...
            first_alpha = s
            break
        cleaned = [w.strip(".,;:()[]\"'").lower() for w in self.text.split()]
        first_word = cleaned[0] if cleaned else ""
        first_two = " ".join(cleaned[:2]) if len(cleaned) >= 2 else ""
        is_recital_trigger = (
            first_word in _RECITAL_TRIGGERS or first_two in _RECITAL_TRIGGERS
        )
        self.is_prose_continuation = (
            self.last_word_is_co_ordinate_conjunction  # ends with comma/conjunction
            or self.continuing_line
            or (bool(first_alpha) and first_alpha[0].islower())
            or is_recital_trigger  # generic recital opener (WHEREAS, NOW THEREFORE)
        )
...
def _cached_bare_line(line_str):
    return Line(line_str)


def bare_line(line_str):
    """A ``Line(line_str)`` with no style/page context, shared via cache.

    Only for the bare single-argument construction (every call site in the
    engine except tests) — a ``Line`` built with ``text_list``/``style_dict``/
    ``page_details`` must stay per-call since its ``visual_line`` differs.
    See the cache block comment above :func:`make_word` for the safety
    argument."""
    if _LINE_CACHE_DISABLED:
        return Line(line_str)
    return _cached_bare_line(line_str)
