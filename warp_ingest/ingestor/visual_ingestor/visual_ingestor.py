...
class Doc:
            ...
            if not change_in_class:
                if line_info["page_idx"] != prev_line_info["page_idx"]:
                    ...
                    if shifts_left and block_buf[0]:
                        ...
                        if buf_text:
                            lp_json = line_parser.bare_line(buf_text).to_json()
                            if (
    ...
    def split_line(self, line_info):
        ...
        header_line_info = {
            ...
            "line_parser": line_parser.bare_line(header_text).to_json(),
        }
        normal_line_info = {
            "box_style": line_info["box_style"],
            ...
            "text": normal_text,
            "page_idx": line_info["page_idx"],
            "line_parser": line_parser.bare_line(normal_text).to_json(),
        }
            ...
            if block["block_type"] == "table_row" and len(block["visual_lines"]) == 2:
                ...
                if collected_row and len(collected_row) > 1:
                    ...
                    for row in collected_row:
                        text = row["text"]
                        # if line_parser.bare_line(text).is_header: # using line parser
        ...
        while i < len(organized_blocks):
            ...
            if a_block["block_type"] in [
                ...
                should_be_header = (
                    ...
                    and not line_props.is_prose_continuation  # prose/clause never a header
                    and (
                ...
                if a_block["block_type"] == "list_item" and should_be_list:
                    ...
                    lp_first_sent = line_parser.bare_line(first_sent)
                ...
                if para_sentence and add_para_block:
                    ...
                    # True in the *run-in* branch below, where the heading and the
                    # body share the first visual line so the header and paragraph
                    # need distinct sub-boxes carved from that line's word geometry.
                    same_line_runin = False
                    ...
                    # Give the header and paragraph *distinct, correct* boxes. The
                    # header used to inherit a_block's whole-block box_style (the
                    # object the paragraph also kept), which made a downstream
                    # geometric token-assigner unable to tell them apart. For a
                    # run-in (heading + body on one physical line) we carve the
                    # shared line's per-word geometry into a header sub-box and a
                    # body sub-box; otherwise each block simply spans its own lines.
                    Doc.split_runin_boxes(header_block, para_block, same_line_runin)
                    j = idx + 1
                    ...
                    if len(para_block["visual_lines"]):
                        para_props = line_parser.bare_line(para_block["block_text"])
        ...
        def check_special_line(block):
            for line_idx, line in enumerate(block["visual_lines"]):
                ...
                if not "line_parser" in line or not line["line_parser"]["noun_chunks"]:
                    line["line_parser"] = line_parser.bare_line(line["text"]).to_json()
    ...
    def make_table_with_footers(
        ...
        if table_start_idx > 0:
            ...
            if (
                ...
                and (left != 0 and prev_box[1] / left > 1.1)  # or is_aligned)
    ...
    def _annotate_block_emphasis(self, block):
        """Surface the real per-word font weight the front-end already extracted
        as additive block fields, *before* ``visual_lines`` are discarded.

        Adds (never removes/changes existing fields, so downstream renderers and
        regression baselines that key on existing fields are unaffected):

        * ``bold_ratio`` -- fraction of the block's words drawn in a bold weight
          (numeric font-weight > 400), from ``word_classes`` -> ``class_line_styles``.
        * ``bold_mask`` -- a ``"0"``/``"1"`` string, one flag per word in the
          block's visual reading order (``"1"`` == bold), so a renderer can wrap
          the exact bold runs (a bold sub-heading, a run-in label like ``"NOTE:"``
          or ``"[0032]"``, or a wholly-bold line).
        * ``italic_ratio`` / ``italic_mask`` -- the exact italic counterpart
          (``line_style`` font-style == ``"italic"``), so a renderer can wrap the
          real italic runs in Markdown ``*emphasis*`` (and bold+italic in
          ``***...***``).

        Renderers can use these to emit genuine inline emphasis (e.g. Markdown
        ``**bold**`` / ``*italic*``) instead of inventing none.  The signal is the
        engine's own extracted font weight/style, not a content heuristic.
        """
        flags = []  # per-word bold flag in visual reading order
        ital = []  # per-word italic flag, same order
        for vl in block.get("visual_lines", []) or []:
            toks = (vl.get("text") or "").split()
            word_classes = vl.get("word_classes") or []
            for i, _tok in enumerate(toks):
                cls = word_classes[i] if i < len(word_classes) else vl.get("class")
                line_style = self.class_line_styles.get(cls)
                flags.append(
                    line_style is not None and line_style[3] and line_style[3] > 400
                )
                ital.append(line_style is not None and line_style[1] == "italic")
        if not flags:
            return
        block["bold_ratio"] = round(sum(1 for b in flags if b) / len(flags), 3)
        if any(flags):
            block["bold_mask"] = "".join("1" if b else "0" for b in flags)
        block["italic_ratio"] = round(sum(1 for b in ital if b) / len(ital), 3)
        if any(ital):
            block["italic_mask"] = "".join("1" if b else "0" for b in ital)

    def compress_blocks(self):
        for block in self.blocks:
            self._annotate_block_emphasis(block)
    ...
    def get_list_item_subtype(block):
        lp = line_parser.bare_line(block["block_text"])
        item_subtype = "bullet"
    ...
    def merge_line_info(first, second, remove_space=False):
        ...
        lp_line = line_parser.bare_line(combined_text)
        combined_block = {
            "box_style": box_style,
    ...
    @staticmethod
    def split_runin_boxes(header_block, para_block, same_line_runin):
        """Give a run-in header and its paragraph *distinct* ``box_style``s, each
        recomputed from that block's own visual lines.

        The run-in split used to hand the header and the paragraph the *same*
        ``BoxStyle`` object (``a_block``'s whole-block span), which left a downstream
        geometric token-assigner unable to tell the two apart — one sibling then
        stole the other's tokens. Recomputing each block's span from its own lines
        removes that (systematic) shared-box defect and fixes the *separate-line*
        run-in ("6.4 Audits." on its own line, body below) at the root, in the
        ``json``/``html``/``all`` renders too.

        The ``same_line_runin`` case — heading and body sharing *one* physical line
        — is intentionally *not* sub-divided here: the engine's line-assembly
        pipeline collapses the per-word geometry that split would need (a 22-word
        run-in line reaches this point with far fewer word extents), so carving a
        header/body sub-box in the engine is unreliable. That residual is repaired
        in the OpenContracts exporter (``_reconcile_runin_overflow``), which
        reconstructs clean per-word boxes from the XHTML. ``same_line_runin`` is
        kept as an explicit signal of that boundary.
        """
        _ = same_line_runin  # documented boundary; handled in the exporter
        if header_block.get("visual_lines"):
            header_block["box_style"] = Doc.calc_block_span(header_block)
        if para_block.get("visual_lines"):
            para_block["box_style"] = Doc.calc_block_span(para_block)
    ...
    def merge_vls(vl_merge_buf):
        ...
        lp_line = line_parser.bare_line(vl_text)
        merged_vl = {
            "text": vl_text,
    ...
    def merge_blocks(self, blocks):
        ...
        lp_line = line_parser.bare_line(blocks[0]["block_text"])
        block_type = (
            "list_item"
            ...
            if split_idx > -1:
                ...
                if ptag_idx > -1:
                    ...
                    if prefix_vls:
                        ...
                        lp_line = line_parser.bare_line(text)
                        line_info = {
                            "box_style": box_style,
        ...
        for blk_idx, blk in enumerate(self.blocks):
            if (
                ...
                for vl_group in same_line_vl_group:
                    ...
                    line_props = line_parser.bare_line(last_line_text)
    ...
    def merge_header_blocks(self):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            ...
            if (
                ...
                json_rec = line_parser.bare_line(translated_str).to_json()
                name_decider = False
