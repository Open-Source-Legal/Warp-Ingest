...
from warp_ingest.ingestor_utils.ing_named_tuples import BoxStyle, LineStyle
...
def _word_line_style(wf, font_family):
    """Cached per-word LineStyle for one ``word-fonts`` tuple string.

    A document uses a handful of fonts, so the same ``(family,weight,style,
    size,size,space_width)`` string repeats for thousands of words; LineStyle
    is an immutable namedtuple, so instances can be shared.  Mirrors the
    original inline construction exactly (including the comma-in-family
    repair keyed on the line's *font_family*)."""
    if "," in font_family and font_family in wf:
        wf = wf.replace(font_family, font_family.replace(",", "-"))
    parts = wf.split(",")
    return LineStyle(
        parts[0],
        parts[2],
        round(font_scale * float(parts[3]), 1),
        get_numeric_font_weight(parts[0], parts[1]),
        "none",
        round(float(parts[5]), 2),
        "left",
    )
...
def parse_tika_style(style_str: str, text_str: str, page_width: float) -> dict:
    ...
    for wf_idx, wf in enumerate(word_fonts):
        ls = _word_line_style(wf, font_family)
        word_line_styles.append(ls)
        if wf_idx == 0:
            font_space_width = ls.font_space_width
