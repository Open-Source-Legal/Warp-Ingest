...
from warp_ingest.ingestor.visual_ingestor import table_parser
from warp_ingest.ingestor_utils.utils import detect_block_center_aligned
    ...
    def indent_blocks(self):
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            def get_level(new_class_name):
                ...
                line = line_parser.bare_line(block["block_text"])
                if (
                ...
                if len(level_stack) == 0:
                    ...
                    # Stack entries are flat dicts of scalars (strings/bools/
                    # floats — see the new_class literal above and every write
                    # to it), so per-entry shallow copies are exactly
                    # equivalent to deepcopy at a fraction of the cost.
                    new_stack = [dict(entry) for entry in level_stack]
