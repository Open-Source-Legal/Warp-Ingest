...
from warp_ingest.ingestor_utils.ing_named_tuples import BoxStyle
...
class TableParser:
    ...
    def _global_col_spans(self, start_idx):
        """Compute a single global column grid for the whole table.

        The legacy path accretes ``col_spans`` row-by-row, so a column boundary
        fixed by early rows never shifts to fit later rows -- wrapped headers and
        ragged rows then land in the wrong column.  Instead, derive the grid once
        from *every* data row at once: a column is a vertical band that cell text
        occupies across the rows, and a column separator is a vertical whitespace
        band left empty by (almost) all rows.  Every row is later assigned to the
        same grid, so cells align.

        Geometry only (box_style = ``[top, left, right, width, height]``).
        Returns a sorted ``[[left, right], ...]`` or ``None`` when the table is
        too sparse / single-column to model confidently (then the caller keeps
        the legacy spans, so behaviour only changes where the grid is clear).
        """
        rows = []
        for block in self.blocks[start_idx:]:
            if block.get("block_type") != "table_row":
                continue
            vls = block.get("visual_lines") or []
            if not vls:
                continue
            # A full-width single-segment row (a spanning title/row-group) does
            # not define columns -- skip it from grid determination.
            if len(vls) == 1 and vls[0]["box_style"][3] > 0.6 * self.doc.page_width:
                continue
            rows.append(vls)
        if len(rows) < 2:
            return None

        lefts = [vl["box_style"][1] for r in rows for vl in r]
        rights = [vl["box_style"][2] for r in rows for vl in r]
        ...
        # Separator support: per bin, how many rows have an inter-cell *gap*
        # (whitespace between two of their cells) covering it. Front-end already
        # split cells at >2.5*font gaps, so every inter-cell gap is a real column
        # break; a bin where enough rows agree there is whitespace is a column
        # separator. Counting gaps (not occupancy) is robust to a few rows whose
        # wide/spanning cell crosses the separator -- they simply don't vote.
        gapsup = [0] * nbins

        def _bin(x):
            return min(nbins - 1, max(0, int((x - x0) / binw)))

        for vls in rows:
            svls = sorted(vls, key=lambda v: v["box_style"][1])
            for a, b in zip(svls, svls[1:]):
                g0, g1 = a["box_style"][2], b["box_style"][1]
                if g1 <= g0:
                    continue
                for bb in range(_bin(g0), _bin(g1)):
                    gapsup[bb] += 1

        # A real column separator is whitespace in (nearly) every row -- a strong
        # majority must agree.  A weaker threshold lets an incidental within-cell
        # gap (present in a minority of rows) split a column, over-segmenting
        # clean tables; requiring a strong majority keeps only true column breaks.
        sep_thr = max(2, int(0.6 * nrows))
        seps = []  # separator-band center x's, left-to-right
        i = 0
        while i < nbins:
            if gapsup[i] >= sep_thr:
                j = i
                while j < nbins and gapsup[j] >= sep_thr:
                    j += 1
                seps.append(x0 + ((i + j) / 2) * binw)
                i = j
            else:
                i += 1
        if not seps:
        ...
        # Build tight column bands: each column's extent is the min-left .. max-
        # right of the cells whose centre falls between two separators (not the
        # full separator-to-separator span), matching the legacy span format the
        # downstream cell-assignment expects.
        bounds = [x0 - 1.0] + seps + [x1 + 1.0]
        ncol = len(bounds) - 1
        col_l = [None] * ncol
        col_r = [None] * ncol
        for vls in rows:
            for vl in vls:
                cx = (vl["box_style"][1] + vl["box_style"][2]) / 2
                k = 0
                while k < ncol - 1 and cx >= bounds[k + 1]:
                    k += 1
                lft, rgt = vl["box_style"][1], vl["box_style"][2]
                col_l[k] = lft if col_l[k] is None else min(col_l[k], lft)
                col_r[k] = rgt if col_r[k] is None else max(col_r[k], rgt)
        cols = [
            [round(col_l[k], 2), round(col_r[k], 2)]
            for k in range(ncol)
            if col_l[k] is not None
        ]
        if len(cols) < 2:
            return None
        # Enforce the sorted, non-overlapping span invariant the downstream cell
        # assignment assumes (align_columns / align_header_group / align_missing_
        # columns all expect ascending, disjoint spans). The tight bands are built
        # left-to-right so they are already sorted, but a cell whose box straddles
        # a majority-voted separator can push one band's max-right past the next
        # band's min-left; clamp any such overlap to its midpoint. Bail to the
        # legacy spans if the clamp leaves a degenerate band.
        for k in range(len(cols) - 1):
            if cols[k][1] > cols[k + 1][0]:
                mid = round((cols[k + 1][0] + cols[k][1]) / 2, 2)
                cols[k][1] = mid
                cols[k + 1][0] = mid
        if any(c[1] <= c[0] for c in cols):
            return None
        # Never claim more columns than the widest row actually has -- that would
        # be spurious over-segmentation (a within-cell gap mistaken for a column).
        max_cells = max(len(r) for r in rows)
        if len(cols) > max_cells:
            return None
        return cols
    ...
    def add_table_data(self):
        ...
        # Prefer a single global column grid derived from all rows at once over
        # the row-by-row accreted spans, when the table's column structure is
        # clear (>= 2 confident columns). This fixes cross-row cell misalignment
        # (wrapped headers / ragged rows) without disturbing tables the global
        # model can't confidently segment (it returns None there -> legacy spans).
        global_spans = self._global_col_spans(span_determine_start_idx)
        if global_spans is not None and len(global_spans) > len(col_spans):
            # Adopt the global grid only when it recovers *more* columns than the
            # legacy row-by-row accretion found. The accretion's failure mode is
            # merging columns (a boundary fixed by early rows never reopens), so
            # "grid has more columns" is exactly the under-segmentation we want to
            # fix; the grid is bounded above by the widest row, so this can't
            # over-segment. When the legacy already has >= the grid's columns it
            # is kept, leaving correctly- and finely-segmented tables untouched.
            col_spans = global_spans
