...
from warp_ingest.ingestor import formatter, line_parser
from warp_ingest.ingestor_utils.word_splitter import WordSplitter
...
def join_words(prev_text_list: list) -> bool:
    if len(prev_text_list) == 1:
        ...
        line_type = line_parser.bare_line(joined_text).line_type
...
def p_to_lines(p_items: list) -> list:
    ...
    for p_item in p_items:
        ...
        if diff_prev_y < (8 / 5) * min_font_size or key_value_check:
            ...
            # print("Line analysis")
            line_type = line_parser.bare_line(text).line_type
            if join_single_letters(line_info["text_list"]):
                # if spaced characters, join to more accurately asses line type
                prev_line_type = line_parser.bare_line(
                    "".join(line_info["text_list"]),
                ).line_type
            else:
                prev_line_type = line_parser.bare_line(prev_text).line_type
