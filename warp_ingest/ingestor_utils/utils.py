...
# Each rule carries a (kind, needle) prefilter: a cheap *necessary* condition
# for the rule's regex to match, so sent_tokenize can skip the regex scan when
# it provably cannot fire.  The abb is interpolated into the pattern raw, so
# its alphanumeric characters are literal (case-insensitive):
#
# - kind "start"  (pattern ``^{abb}.\s``): the text must *begin* with the
#   abb's leading alphanumeric run -> ``text.startswith(needle)``.
# - kind "word"   (pattern ``\s{abb}.\s``): the run must appear immediately
#   after a whitespace character -> _space_anchored().  Anchoring matters:
#   short abbs ("no", "st") occur as bare substrings in most English text,
#   but rarely as a token start, so this prunes ~all non-firing rules.
# - kind "any"    (pattern ``{abb}\.``): plain substring of the longest run.
#
# A needle of None means "always run the rule" (no alphanumeric lead to key on).


def _leading_needle(abb):
    m = re.match(r"[a-z0-9]+", abb.casefold())
    return m.group(0) if m else None


def _any_needle(abb):
    runs = re.findall(r"[a-z0-9]{2,}", abb.casefold())
    return max(runs, key=len) if runs else None


def _space_anchored(text, needle):
    """True iff *needle* occurs at the start of *text* or right after a
    whitespace character — the only placements where a ``\\s{abb}...`` rule
    can fire (start-of-text included to stay conservative)."""
    i = text.find(needle)
    while i != -1:
...
for abb in abbs:
    ...
    rules.append(("start", _leading_needle(abb), rule, replaced))
    ...
    # case insensitive replacement for synonyms
    rule = re.compile(pattern, re.IGNORECASE)
    rules.append(("word", _leading_needle(abb), rule, replaced))

for abb in nlm_special_abbs:
    pattern = rf"{abb}\."
    replaced = f"{abb}_"
    rule = re.compile(pattern, re.IGNORECASE)
    rules.append(("any", _any_needle(abb), rule, replaced))
...
def sent_tokenize(org_texts):
    ...
    for org_text in org_texts.split("\n"):
        ...
        # Skip rules whose abbreviation cannot occur in this text: each check
        # is a pure necessary condition for its pattern shape (see the rules
        # block comment), so the surviving rules produce the same result as
        # running all of them — with ~all non-firing regex scans pruned.
        text_cf = modified_text.casefold()
        for kind, needle, rule, replaced in rules:
            if needle is not None:
                if kind == "start":
                    if not text_cf.startswith(needle):
                        continue
                elif kind == "word":
                    if not _space_anchored(text_cf, needle):
                        continue
                elif needle not in text_cf:
                    continue
            new_text = rule.sub(replaced, modified_text)
