...
from warp_ingest.ingestor import line_parser as lp
...
class MyTest(unittest.TestCase):
    ...
    def test_check_date(self):
        # Real date-shaped tokens are flagged.
        self.assertTrue(lp.Word("10-12").is_date_entry)
        self.assertTrue(lp.Word("10-12-2024").is_date_entry)
        self.assertTrue(lp.Word("1/5").is_date_entry)
        self.assertTrue(lp.Word("12/25/2024").is_date_entry)
        self.assertTrue(lp.Word("Jan-5").is_date_entry)
        # Digit-free hyphenated words must NOT be dates (the strptime-storm gate:
        # every date pattern needs a numeric field, so these short-circuit).
        self.assertFalse(lp.Word("well-known").is_date_entry)
        self.assertFalse(lp.Word("e-mail").is_date_entry)
        self.assertFalse(lp.Word("pre-existing").is_date_entry)
        self.assertFalse(lp.Word("co-founder").is_date_entry)
        # Hyphenated tokens that DO contain a digit but are not dates stay False.
        self.assertFalse(lp.Word("EX-10").is_date_entry)
        self.assertFalse(lp.Word("10-Q").is_date_entry)
        # No separator -> never a date.
        self.assertFalse(lp.Word("September").is_date_entry)
...
def test_prose_continuation_flag():
    assert Line(
        "WHEREAS, the current renewal term expires September 30,"
    ).is_prose_continuation
    assert Line("herein, shall remain in full force and effect.").is_prose_continuation
    assert Line(
        "WHEREAS, the CONTRACT involves engineering services for"
    ).is_prose_continuation
    # real headings are NOT continuations
    assert not Line("AMENDMENTS").is_prose_continuation
    assert not Line("ARTICLE 9").is_prose_continuation
    assert not Line("Indemnification and Insurance").is_prose_continuation
    # statute / Title-case headings must NOT be flagged as prose continuations
    assert not Line(
    ...
    # recital opener IS a continuation (generic trigger)
    assert Line(
...
_CACHE_PROBE_TEXTS = [
    ...
    "This Agreement is made as of January 1, 2024, by and between the parties.",
    "• first bullet item",
    ...
    "Our executive offices are located at 601 N.w.",
    "TABLE OF CONTENTS",
    "(a) each Party shall keep confidential;",
]


def test_make_word_cache_equivalence():
    # a cached Word must carry exactly the analysis a fresh Word computes
    for tok in _CACHE_PROBE_TOKENS:
        fresh = lp.Word(tok)
        cached = lp.make_word(tok)
        assert cached.__dict__ == fresh.__dict__, tok
        # repeated lookups keep returning the same analysis
        assert lp.make_word(tok).__dict__ == fresh.__dict__, tok


def test_bare_line_cache_equivalence():
    # bare_line must be indistinguishable from a fresh style-free Line
    for text in _CACHE_PROBE_TEXTS:
        fresh = lp.Line(text)
        cached = lp.bare_line(text)
        assert cached.to_json() == fresh.to_json(), text
        # the full attribute surface (minus object-valued fields) agrees
        for attr, val in fresh.__dict__.items():
            if attr in ("visual_line", "words"):
                continue
            assert getattr(cached, attr) == val, (text, attr)


def test_bare_line_cache_interleaving_is_stable():
    # a busy cache must never bleed state between different texts
    first = {t: lp.bare_line(t).to_json() for t in _CACHE_PROBE_TEXTS}
    for t in reversed(_CACHE_PROBE_TEXTS):
        assert lp.bare_line(t).to_json() == first[t], t
